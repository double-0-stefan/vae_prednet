...
class pc_conv_network_basic(nn.Module):
	'''
	super-basic version of this without any VAE structures
	works like Bogacz paper but with convolutional covariance approach

	First thing: second order optimisation based on ?Fisher scoring or DNN similar (like DEM)

	Second thing to add after up and running: Descending expected precision networks?

	The covariance networks are in a sense 'hardwired' - they are learned synaptic parameters
	Descending precision network will want to modulate expected precision of an individual image.

	This expected precision will be refined/learned over one inner/percuptual loop, just like the normal states

	Could simply do this diagonally, but prob better to do non-diag.

	This will perhaps look like a CNN from standard states to (expected precision states to) expanded CNN weights
	in the conv_cov matrix. Thus some of these *will* be modified at perception-time, and also weights assigned to
	same underlying parameter may be modified differently
	The expected precision *could* work in a similar way to the conv_cov matrix (cf, variance/precision components -
	this is the way DEM does it, I think) with a smaller number of underlying parameters. These would be mapped in
	a different way though - perhaps all states generated by an individual component (perhaps defined across filter 
	space - eg position in image) of the level above could share a parameter (??into their mixture??). Or be influenced 
	by neighbouring pixels (?need symmetry here too). This could be represented as a conv (input=many, output=1) filter 
	(of size 1, or size > 1 if wanted neighbours to influence each other)
	...
	def __init__(self,p):
		...
		self.init_conv_trans_desc(p)

		self.init_conv_cov(p)

		self.optimizer = None
		# print(self)


	def init_conv_trans_desc(self, p): 
		...
		# Create network
		# can have multiple convolutions in a 'layer'
			...
			for i in reversed(range(len(p['ks'][j]))):
				if i == 0:
					...
					# set p[pad] to off
						...
						# this isn't stored ultimately but used to work out image sizes
	...
	def init_covariance(self, p): 
		...
		# bottom level
		Covariance.append(
			conv2d_cov(in_channels=self.p['imchan'], out_channels=self.p['imchan'], 
				kernel_size= self.p['cov_size'][0],
				stride=1, 
				padding= int((self.p['cov_size'][0]-1)/2),
				dilation=1, groups=1, bias=None, image_size=self.p['imdim'] , padding_mode='zero')
					)
		for i in range(len(self.phi)):		
				Covariance.append(
					conv2d_cov(in_channels=self.p['chan'][i][-1], out_channels=self.p['chan'][i][-1],
						kernel_size= self.p['cov_size'][i],
						stride=1, 
						padding= int((self.p['cov_size'][i]-1)/2),
						dilation=1, groups=1, bias=None, padding_mode='zero')
					)
		self.Covariance = nn.ModuleList(Covariance).cuda()
	...
	def loss(self, i, learn=0):
		...
		if learn == 0:
			...
			self.Covariance.requires_grad_(True)

		for i in range(-1, self.nlayers): # -1):

			# Send layer above through the trans_conv
			x = self.conv_trans[i+1](self.phi[i+1].view(self.bs, self.chan[i+1][-1], self.dim[i+1][-1], self.dim[i+1][-1])).view(self.bs,-1)

			if i = -1:
				PE.append(self.images - x)
				if self.eval_:
					self.pred = x.view(self.bs,1,32,32)
			else:
				PE.append(self.phi[i] - x)

			f.append(
				sum(sum(
					PE[i+1].view(-1).mm(self.Covariance[i+1](PE[i+1]).view(-1))
				))/self.p['bs'])
			...
			if learn == 0:
				...
				F[i+1].backward() # f.backward(retain_graph=True)
				self.opt_phi[i].step()

			if learn == 1:
				# calculate negative logdet
				f_ld.append(self.Covariance[i+1].logdet_cov_matrix())
				F.append(torch.sum(f[i+1], f_lf[i+1]))

				self.opt_Covariance[i].zero_grad()
				F[i+1].backward()
				self.opt_Covariance[i].step()
	...
	def forward(self, iteration, images, act=None, eval=False):
		...
		self.opt_Covariance = [None] * (len(self.phi) +1)

		for i in range(len(self.phi)):
			self.opt_phi[i] = Adam([self.phi[i]], lr=self.p['lr'])
			self.opt_Covariance[i]   = Adam([self.Covariance[i]], lr=self.p['lr'])
...
class pc_conv_network_cool_stuff_pre_nov_2020(nn.Module):
	def __init__(self,p):
		super(pc_conv_network, self).__init__()
...
class pc_conv_network_other(nn.Module):
	def __init__(self,p):
		super(pc_conv_network, self).__init__()
