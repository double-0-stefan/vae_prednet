...
class pc_conv_network(nn.Module):
	...
	def loss(self, i, learn=0):
		...
		if self.p['include_precision']:
			...
			if learn == 1:
				print(-self.p['bs'] * self.Precision[i+1].log_det(i+1, PE.view(self.p['bs'], -1).size(1)))
				f = 0.5*sum(sum(
					- self.p['bs'] * self.Precision[i+1].log_det(i+1, PE.view(self.p['bs'], -1).size(1)) # -ve here because more precise = good (nb will need to balance over layers somehow)
