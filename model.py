	...
	def generate_cov_matrix(self):
		...
		for i in range(self.out_channels):
			...
			for k in range(1, middle):
				for j in range(self.out_channels):
					if j < len(self.weight_values) - len(self.weight_values[i]):
						rhs[i,4*k*self.out_channels + 4*j : 4*k*self.out_channels + 4*j +4] = self.weight_values[j][-(1+k),i]#.view(1,-1).expand(4*k,-1)
					else:
						rhs[i,4*k*self.out_channels + 4*j : 4*k*self.out_channels + 4*j +4] = self.weight_values[i][-(1+k),j]#.view(1,-1).expand(4*k,-1)
