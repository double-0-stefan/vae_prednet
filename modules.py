...
class pc_conv_layer(Module):
	def __init__(self,p,l):
		...
		phi # descending prior

		self.Sigma_u = torch.nn.Parameter(torch.zeros([self.dim,self.dim]))
		self.u = torch.nn.Parameter(torch.zeros([self.dim])) # unsure if need 1
	...
	def loss(self):
		...
		F = 0.5*(	#- torch.logdet(Sigma_p)
					...
					- torch.logdet(Sigma_u)
					- torch.matmul(
						torch.matmul(						
							torch.transpose(u - self.conv_trans(phi),0,1),   # g(phi,Theta) = Theta*h(phi) = conv_trans(h(phi))
							torch.inverse(Sigma_u)				# leaving h aside for now
						),
						u - self.conv_trans(phi) # leaving h aside for now
					)
				)
